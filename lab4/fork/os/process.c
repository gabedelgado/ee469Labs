...
int ProcessRealFork(){
  ...
  //check if freepcbs is empty
  if (AQueueEmpty(&freepcbs)){
    exitsim();
  }
  //Child pcb from freepcbs
  ...
  //loop through currentPCB and mark valid ones as READONLY
  for (i=0;i < MEM_L1TABLE_SIZE; ++i){
    currentPage = currentPCB->pagetable[i];
    if (currentPage & MEM_PTE_VALID){ //check if pte is valid
      currentPCB->pagetable[i] = currentPage | MEM_PTE_READONLY;//set to readonly
      IncreaseRefCount(currentPCB->pagetable[i]); //increase ref count for page
    }
  }

  //duplicate PCB using bcopy
  ...
  printf("in fork call, PARENT (%d)page table: \n", GetPidFromAddress(currentPCB));
  for (i = 0; i < MEM_L1TABLE_SIZE; i++){
    if ((currentPCB->pagetable[i] & MEM_PTE_VALID) == 1){
      printf("page %d: %d\n", i, currentPCB->pagetable[i]);
    }
  }
  printf("in fork call, CHILD (%d) page table: \n", GetPidFromAddress(child));
  for (i = 0; i < MEM_L1TABLE_SIZE; i++){
