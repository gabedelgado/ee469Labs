...
void * malloc(PCB * pcb, int memsize){
  ...
  //Check if memsize size exceeds mem_pagesize or is less than 0
  if (memsize <= 0 || memsize > MEM_PAGESIZE){
    return NULL;
  }

  //Find suitable node
  block = NodeSearch(&(pcb->heap_array[1]),memsize);

  if (block >= 0){
    size = pcb->heap_array[block].size;
    vAddr = (MEM_PAGESIZE * 4) | block;
    //Calculate phys addr
    page = vAddr >> MEM_L1FIELD_FIRST_BITNUM;
    offset = vAddr & MEM_ADDRESS_OFFSET_MASK;
    //Check if page is valid
    if (pcb->pagetable[page] & MEM_PTE_VALID){
      physAddr = (pcb->pagetable[page] & MEM_PTE_MASK | offset);
    }
    else{
      physAddr = 0;
      return NULL;
  ...
  //split to get a suitable node
  block = SplitNode(&(pcb->heap_array[1]),pcb,memsize);
  if (block >= 0){
    size = pcb->heap_array[block].size;
    vAddr = (MEM_PAGESIZE * 4) | block;
    //Calculate phys addr
...
int SplitNode(Node * node, PCB * pcb, int memsize){
  ...
  if(node->inUse == 0){ //Check if node is used
    if(node->left == NULL){ //check if we are on the leftmost child or if left node is empty

      if((memsize <= node->size) && (memsize > (node->size / 2))){ //check is size to allocate is valid
        node->inUse = 1;
      ...
      if ((node->size / 2) < memsize){
        ...
        if (node->order == 0) {
          ...
          //Create left child
          leftChild = &(pcb->heap_array[2*node->index]);
          leftChild->parent = node;
          leftChild->left = NULL;
          leftChild->right = NULL;
          leftChild->size = node->size/2;
          leftChild->order = node->order - 1;
          leftChild->addr = node->addr;
          printf("Created a left child node (order = %d, addr = %d, size = %d) of parent (order = %d, addr = %d, size = %d)\n",leftChild->order,leftChild->addr,leftChild->size,node->order,node->addr,node->size);
          
          //Create right child
          rightChild = &(pcb->heap_array[2*node->index]+1);
          rightChild->parent = node;
          rightChild->left = NULL;
          rightChild->right = NULL;
          rightChild->size = node->size/2;
          rightChild->order = node->order - 1;
          rightChild->addr = node->addr + rightChild->size;
          printf("Created a right child node (order = %d, addr = %d, size = %d) of parent (order = %d, addr = %d, size = %d)\n",rightChild->order,rightChild->addr,rightChild->size,node->order,node->addr,node->size);
  ...
  //Traverse through tree via recursion
  temp = SplitNode(node->left,pcb,memsize);
  if (temp >= 0){
    return temp;
  }
  else{
    return SplitNode(node->right,pcb,memsize);
  }
}
...
int NodeSearch(Node * node, int memsize){
  ...
  temp = NodeSearch(node->left, memsize);
  if (temp >= 0){
    return temp;
  }
  else{
    return NodeSearch(node->right,memsize)
  }
}
